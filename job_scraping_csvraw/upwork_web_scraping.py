...
def generate_url(job, page):
    template_url = "https://www.upwork.com/nx/jobs/search/?q={}&sort=relevance&page={}"
    final_url = template_url.format(job,page)
    return final_url

url = generate_url(jobs, page)
time.sleep(3)
s.get(url)
time.sleep(6)  
